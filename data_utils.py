...
def generate_dataset(data_type, upscale_factor):
    ori_path = './your_dataset/'
    
    images_name = [x for x in listdir(ori_path) if is_image_file(x)]
    crop_size = calculate_valid_crop_size(1023, upscale_factor)
    ...
    for image_name in tqdm(images_name, desc='generate ' + data_type + ' dataset with upscale factor = '
        ...
        data = image.copy()
        target = hr_transform(image)
        image = lr_transform(data)
        

        image.save(image_path + '/' + image_name)
        target.save(ori_path + '/' + image_name)
...
if __name__ == "__main__":
    ...
    generate_dataset(data_type='data', upscale_factor=UPSCALE_FACTOR)#train
    generate_dataset(data_type='target', upscale_factor=UPSCALE_FACTOR)#value
    generate_dataset(data_type='test', upscale_factor=UPSCALE_FACTOR)#test